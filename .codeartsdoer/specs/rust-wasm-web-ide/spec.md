...
2. 用户操作指令（运行、保存、打开文件、启用类型检查）
3. 项目配置文件（Cargo\.toml）
4. GPU 执行优化配置参数
5. rust-analyzer 功能开关（可选启用）
...
2. 错误提示和定位信息
3. 类型检查结果和诊断信息（可选）
4. 持久化的用户文件（存储于 localStorage）
5. 发布编译生成的 Wasm 产物（未来功能）
...
1. Rust 解释器（syn）：在浏览器内解释执行 Rust 源码，提供即时运行反馈
2. rust-analyzer（可选）：提供类型检查、类型推断、智能补全、错误诊断功能
3. WebGPU API：提供 GPU 并行计算能力，用于执行优化
4. Service Worker：支持多域名下的 JS 线程管理和资源加载
...
2. **自动补全规则**：编辑器必须提供 Rust 代码自动补全，包括内置函数、标准库 API、用户定义函数等。补全分为两层：
   - 基础补全（syn）：关键字、内置函数、简单语法结构（如 fn、let、if 等）
   - 智能补全（rust-analyzer）：上下文感知补全、类型推断补全、方法补全（当 rust-analyzer 启用时优先使用）

   a. 验收条件：[用户输入 "pri"] → [自动提示 "print\!"、"println\!" 等候选项]
   
   b. 验收条件：[启用 rust-analyzer 且用户输入 "x."] → [显示 x 的所有方法和字段（基于类型推断）]
...
2. **类型检查失败**
   ...
   a. 触发条件：[用户启用 rust-analyzer 且代码存在类型错误]

   b. 系统行为：[rust-analyzer 返回类型错误诊断，标记错误位置]

   c. 用户感知：[编辑器中显示类型错误提示和修复建议]
...
1. **懒加载规则**：rust-analyzer 应按需加载，不阻塞首次使用体验。

   a. 验收条件：[用户首次访问] → [仅加载 syn（500KB），用户可立即开始编码]

2. **启用触发规则**：用户主动启用类型检查时，系统才加载 rust-analyzer（约 4MB）。

   a. 验收条件：[用户点击"启用类型检查"] → [开始加载 rust-analyzer，完成后自动执行类型检查]

3. **类型检查规则**：必须支持完整的 Rust 类型系统检查，包括泛型、生命周期、Trait 等。

   a. 验收条件：[代码存在类型错误] → [显示详细的类型错误诊断，包括错误位置和修复建议]

4. **类型推断规则**：必须支持类型推断，为变量、表达式推断类型。

   a. 验收条件：[鼠标悬停在变量上] → [显示推断的类型信息]

5. **增量分析规则**：代码修改后应增量更新分析结果，避免全量重新分析。

   a. 验收条件：[用户修改代码] → [增量分析时间 < 100ms]

6. **降级规则**：当 rust-analyzer 加载失败时，系统应降级为 syn 的基础功能。

   a. 验收条件：[rust-analyzer 加载失败] → [提示用户，继续使用 syn 执行功能]
...
actor "开发者" as Dev
rectangle "IDE" as IDE
rectangle "rust-analyzer" as RA

Dev -> IDE : 点击"启用类型检查"
IDE -> RA : 懒加载 rust-analyzer（4MB）
RA --> IDE : 加载完成
IDE -> RA : 传递 Rust 源代码
RA -> RA : 类型检查
RA -> RA : 类型推断
...
1. **rust-analyzer 加载失败**

   a. 触发条件：[网络问题或 Wasm 加载超时]

   b. 系统行为：[降级为 syn 基础功能，记录错误日志]

   c. 用户感知：[提示"类型检查不可用，可继续使用基础功能"]

2. **类型检查超时**

   a. 触发条件：[代码复杂度过高，类型检查超过 5 秒]

   b. 系统行为：[显示进度提示，允许用户取消]

   c. 用户感知：[显示"正在分析中..."进度条]

3. **内存不足**

   a. 触发条件：[rust-analyzer 占用内存超过限制]

   b. 系统行为：[释放部分缓存，提示用户减少代码量]

   c. 用户感知：[提示"内存不足，建议减少代码复杂度"]
...
2. **任务分配规则**：当启用多线程时，系统必须将代码分析任务（如语法校验、类型检查、自动补全）分配到不同线程并行执行。rust-analyzer 的分析任务可利用多线程加速。

   a. 验收条件：[编辑大型代码文件且启用 rust-analyzer] → [rust-analyzer 任务在多线程中并行执行，响应时间缩短]
