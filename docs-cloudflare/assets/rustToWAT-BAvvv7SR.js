...
`);for(let n=0;n<s.length;n++)s[n].trim().startsWith("fn ")&&this.parseFunction(s,n)}parseFunction(t,s){const n=t[s].trim(),o=n.match(/fn\s+(\w+)\s*\(([^)]*)\)/);if(!o)return s;const i=o[1],e=this.parseParams(o[2]);let r=[],c=0,l=s,h=!1;for(;l<t.length;){const a=t[l];if(a.includes("{")&&(h=!0,c+=(a.match(/{/g)||[]).length),a.includes("}")&&(c-=(a.match(/}/g)||[]).length),h&&(r.push(a),c===0))break;l++}const u=this.convertFunction(i,e,r);return this.module.functions.push(u),(i==="main"||n.includes("#[wasm_bindgen]"))&&this.module.exports.push(`(export "${i}" (func $${i}))`),l}parseParams(t){return t.trim()?t.split(",").map(s=>s.trim().split(":")[0].trim()).filter(s=>s&&s!=="self"):[]}convertFunction(t,s,n){this.localVars=new Map;let o=s.length,i=`(func $${t}`;s.forEach((c,l)=>{i+=` (param $${c} i32)`,this.localVars.set(c,l)}),i+=` (result i32)
`;const e=n.slice(1,-1),r=this.convertBody(e,o);return i+=r,i+=`
    i32.const 0
  )`,i}convertBody(t,s){let n="";for(const o of t){const i=o.trim();if(i.startsWith("let ")){const e=i.match(/let\s+(mut\s+)?(\w+)\s*=\s*(.+);/);if(e){const r=e[2],c=e[3];this.localVars.set(r,s++),n+=`    (local $${r} i32)
`,n+=this.convertExpression(c),n+=`    local.set $${r}
`}}else if(i.startsWith("println!"))n+=this.convertPrintln(i);else if(i.startsWith("return ")){const e=i.match(/return\s+(.+);/);e&&(n+=this.convertExpression(e[1]))}else if(i.includes(" = ")&&!i.startsWith("let")){const e=i.match(/(\w+)\s*=\s*(.+);/);if(e){const r=e[1],c=e[2];n+=this.convertExpression(c),n+=`    local.set $${r}
