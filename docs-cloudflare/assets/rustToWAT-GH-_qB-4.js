...
`);for(let i=0;i<t.length;i++){const e=t[i].trim();e.startsWith("fn ")?this.parseFunction(t,i):e.startsWith("impl ")&&this.parseImpl(t,i)}}parseImpl(s,t){const e=s[t].trim().match(/impl\s+(?:(\w+)\s+for\s+)?(\w+)\s*\{?/);if(!e)return t;e[1];const o=e[2];let n=0,r=t,c=!1;for(;r<s.length;){const l=s[r];if(l.includes("{")&&(c=!0,n+=(l.match(/{/g)||[]).length),l.includes("}")&&(n-=(l.match(/}/g)||[]).length),c&&n===0)break;r++}const h=s.slice(t+1,r);for(const l of h){const m=l.trim();if(m.startsWith("fn ")){const a=m.match(/fn\s+(\w+)\s*\(([^)]*)\)/);if(a){const f=a[1];a[2],this.module.functions.push(`  ;; Method ${o}.${f}
`)}}}return r}parseFunction(s,t){const i=s[t].trim(),e=i.match(/fn\s+(\w+)\s*\(([^)]*)\)/);if(!e)return t;const o=e[1],n=this.parseParams(e[2]);let r=0,c=t,h=!1,l=0;for(;c<s.length;){const f=s[c];if(f.includes("{")&&(h||(l=c),h=!0,r+=(f.match(/{/g)||[]).length),f.includes("}")&&(r-=(f.match(/}/g)||[]).length),h&&r===0)break;c++}const m=s.slice(l,c+1),a=this.convertFunction(o,n,m);return this.module.functions.push(a),(o==="main"||i.includes("#[wasm_bindgen]"))&&this.module.exports.push(`(export "${o}" (func $${o}))`),c}parseParams(s){return s.trim()?s.split(",").map(t=>t.trim().split(":")[0].trim()).filter(t=>t&&t!=="self"):[]}convertFunction(s,t,i){this.localVars=new Map;let e=t.length;t.forEach((h,l)=>{this.localVars.set(h,l)});const o=i.join(`
...
`,n}convertMatch(s,t){const e=s[t].trim().match(/match\s+(.+)\s*\{/);if(!e)return{wat:"",endIndex:t};const o=e[1];let n=0,r=t,c=!1;for(;r<s.length;){const g=s[r];if(g.includes("{")&&(c=!0,n+=(g.match(/{/g)||[]).length),g.includes("}")&&(n-=(g.match(/}/g)||[]).length),c&&n===0)break;r++}const h=s.slice(t+1,r),l=[];let m="",a=[],f=0,p=!1;for(const g of h){const d=g.trim();d.endsWith("=>")&&!p?(m=d.replace(/\s*=>\s*$/,"").trim(),p=!0,f=0):p&&(d==="{"?f++:d==="}"?(f--,f===0&&(l.push({pattern:m,body:a}),a=[],p=!1)):d.endsWith(",")&&f===0?(a.push(d.replace(/,\s*$/,"")),l.push({pattern:m,body:a}),a=[],p=!1):a.push(d))}let u="";u+=this.convertExpression(o);for(let g=0;g<l.length;g++){const d=l[g];d.pattern==="_"?u+=this.convertBody(d.body):(u+=`    i32.const ${d.pattern}
`,u+=`    i32.eq
`,u+=`    (if
`,u+=`      (then
`,u+=this.convertBody(d.body),u+=`      )
`,u+=`    )
`)}return{wat:u,endIndex:r}}convertForLoop(s,t){const e=s[t].trim().match(/for\s+(\w+)\s+in\s+(\d+)\.\.(\d+)\s*\{/);if(!e)return{wat:"",endIndex:t};const o=e[1],n=parseInt(e[2]),r=parseInt(e[3]);let c=0,h=t,l=!1;for(;h<s.length;){const f=s[h];if(f.includes("{")&&(l=!0,c+=(f.match(/{/g)||[]).length),f.includes("}")&&(c-=(f.match(/}/g)||[]).length),l&&c===0)break;h++}const m=s.slice(t+1,h);let a="";return a+=`    i32.const ${n}
`,a+=`    local.set $${o}
`,a+=`    (block $for_end_${t}
`,a+=`      (loop $for_start_${t}
`,a+=`        local.get $${o}
...
`,a+=`        i32.ge_s
`,a+=`        br_if $for_end_${t}
`,a+=this.convertBody(m),a+=`        local.get $${o}
`,a+=`        i32.const 1
`,a+=`        i32.add
`,a+=`        local.set $${o}
`,a+=`        br $for_start_${t}
`,a+=`      )
`,a+=`    )
`,{wat:a,endIndex:h}}convertIfElse(s,t){const e=s[t].trim().match(/if\s+(.+)\s*\{/);if(!e)return{wat:"",endIndex:t};const o=e[1];let n=0,r=t,c=!1;for(;r<s.length;){const f=s[r];if(f.includes("{")&&(c=!0,n+=(f.match(/{/g)||[]).length),f.includes("}")&&(n-=(f.match(/}/g)||[]).length),c&&n===0)break;r++}const h=s.slice(t+1,r);let l=[],m=r;if(r+1<s.length){const f=s[r+1].trim();if(f==="else {"||f.startsWith("} else {")){let p=r+1;for(n=0,c=!1;p<s.length;){const u=s[p];if(u.includes("{")&&(c=!0,n+=(u.match(/{/g)||[]).length),u.includes("}")&&(n-=(u.match(/}/g)||[]).length),c&&n===0)break;p++}l=s.slice(r+2,p),m=p}}let a="";return a+=this.convertExpression(o),a+=`    (if
`,a+=`      (then
`,a+=this.convertBody(h),a+=`      )
`,l.length>0&&(a+=`      (else
`,a+=this.convertBody(l),a+=`      )
`),a+=`    )
`,{wat:a,endIndex:m}}convertWhileLoop(s,t){const e=s[t].trim().match(/while\s+(.+)\s*\{/);if(!e)return{wat:"",endIndex:t};const o=e[1];let n=0,r=t,c=!1;for(;r<s.length;){const m=s[r];if(m.includes("{")&&(c=!0,n+=(m.match(/{/g)||[]).length),m.includes("}")&&(n-=(m.match(/}/g)||[]).length),c&&n===0)break;r++}const h=s.slice(t+1,r);let l="";return l+=`    (block $while_end_${t}
...
`,o}}if(s.includes(".")&&s.includes("(")&&s.includes(")")&&!s.includes("..")){const t=s.match(/(\w+)\.(\w+)\s*\(([^)]*)\)/);if(t){const i=t[1],e=t[2],o=t[3].split(",").map(r=>r.trim()).filter(r=>r);let n="";return n+=this.convertExpression(i),o.forEach(r=>{n+=this.convertExpression(r)}),n+=`    call $${i}_${e}
`,n}}if(s.includes("<")&&!s.includes("<<")){const[t,i]=s.split("<").map(e=>e.trim());return this.convertExpression(t)+this.convertExpression(i)+`    i32.lt_s
